...
public class DbAsyncExecutor<T> extends HandlerThread implements AsyncExecutor<T>
{
    ...
    @Override
    public void insertEntry(@NonNull T entry, @NonNull ExecuteController<Void> controller) {
        mWorkerHandler.post(() -> {
            controller.onStart();
    ...
    @Override
    public void removeEntryById(int id, @NonNull ExecuteController<Void> controller) {
        mWorkerHandler.post(() -> {
            controller.onStart();
